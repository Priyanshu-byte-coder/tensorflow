...
PlanHoistBitcastUpwardsToCallers(const HloInstruction* bitcast) {
  ...
  auto set_result_shape =
    ...
    for (HloInstruction* instruction : instructions) {
      ...
      detail::CopyElementType(instruction->shape(), &new_shape);
  ...
  for (HloInstruction* instruction :
    ...
    switch (instruction->opcode()) {
      ...
      case HloOpcode::kBroadcast: {
        TF_ASSIGN_OR_RETURN(
            detail::BitcastParams params,
            detail::CalculateBitcastOfBroadcast(
      ...
      case HloOpcode::kTranspose: {
        TF_ASSIGN_OR_RETURN(
            detail::BitcastParams params,
            detail::CalculateBitcastOfTranspose(
...
absl::StatusOr<Shape> ComputeRootShapeAfterHoistingBitcasts(
  ...
  for (HloInstruction* instruction : GetConsumerSet(dot)) {
    ...
    TF_ASSIGN_OR_RETURN(Shape result_shape, [&]() -> absl::StatusOr<Shape> {
      switch (instruction->opcode()) {
        case HloOpcode::kBroadcast: {
          auto paramsOr = detail::CalculateBroadcastOfBitcast(
        ...
        case HloOpcode::kTranspose: {
          auto paramsOr = detail::CalculateTransposeOfBitcast(
    ...
    if (instruction->IsRoot()) {
      detail::CopyElementType(instruction->shape(), &result_shape);
      return result_shape;
    }
  ...
  for (auto [instruction, result_shape] : rewrite_plan) {
    ...
    switch (instruction->opcode()) {
      ...
      case HloOpcode::kBroadcast: {
        ...
        auto params =
            detail::CalculateBitcastOfBroadcast(broadcast, result_shape);
      ...
      case HloOpcode::kTranspose: {
        ...
        auto params =
            detail::CalculateBitcastOfTranspose(transpose, result_shape);
