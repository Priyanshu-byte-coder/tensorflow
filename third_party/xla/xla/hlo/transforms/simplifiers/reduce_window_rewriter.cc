...
absl::StatusOr<bool> ReduceWindowRewriter::TryOptimizeAssociativeScan(
  ...
  // Replace all scan users (which are index=0 GTEs or dead) with the result.
  for (HloInstruction* user : scan->users()) {
    TF_RETURN_IF_ERROR(user->ReplaceAllUsesWith(result));
